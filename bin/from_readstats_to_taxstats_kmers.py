...
def main():
    ...
    #-------------------------HERE START!!!!!!!!!!!!!!!!!!!!-------------------#
    # acceleration worked, 6 hours faster on a hard test set
    
    
    log_time_ram("Getting all full sequences")
    taxids = list(rank_dict.keys())
    print(len(rank_dict.keys()))
    
    fullseqs = dict()
    
    for seq in km.fasta_read(db_fasta):
        seq_rank_taxid = get_rank_taxid(seq.taxid, args.rank, nodes.nodedict)
        if seq_rank_taxid in taxids:
            fullseqs[seq.shortname] = seq.seq
            
    log_time_ram("Length fullseqs: %d, lenth taxids: %d"%(len(fullseqs), len(taxids)))

    #-------------------------HERE END!!!!!!!!!!!!!!!!!!!!-------------------#
    
    log_time_ram("Processing main match samfile (sampling kmers)")
    
    
    
    kmer_index_dict = dict()
    for ref, read, sequence in km.parse_sam_simple_target(args.samfile, fullseqs, target_read_dict):
        taxid = ref.split("as")[0].strip("sp")
        rank_taxid = get_rank_taxid(taxid, args.rank, nodes.nodedict)
        
        if not rank_taxid in rank_dict:
            continue
        
        if not rank_taxid in kmer_index_dict:
            kmer_index_dict[rank_taxid] = []
        
        target_reads = rank_dict[rank_taxid].readnames
        if read in target_reads:    
            kmer_index_dict[rank_taxid].append([read,sequence]) # read and reference sequence covered in alignment
            
    log_time_ram("Number of taxids in with kmers: %d"%len(kmer_index_dict))
    ...
    for key,val in rank_dict.items():
        ...
        taxid_reads = []
        if key in kmer_index_dict:
            taxid_reads = kmer_index_dict[key]
            
        for dataset in [val,cutval_HPC,cutval_NC, cutval_all]:
            # aus dem kmer_dict die noetigen reads rausziehen, und diese dann berechnen
            kmer_index = "-"
            kmer_dict = dict() # remove later
            kmer_set = set()
            all_kmers = 0
            
            for read,sequence in taxid_reads:
                if read in dataset.readnames:
                    read_kmers = max(0, len(sequence) - k + 1) 
                    all_kmers +=  read_kmers
                    ...
                    for i in range(0, len(sequence)-k+1):
                        ...
                        kmer_set.add(kmer)
                    '''
                        rc_kmer = km.reverse_complement(kmer)
                        kmer_id = min(kmer, rc_kmer)
                    
                        
                        
                        if not kmer_id in kmer_dict:
                            kmer_dict[kmer_id] = 0
                        kmer_dict[kmer_id] += 1
                        
                    uniq_kmers = len(kmer_dict.keys())
                    all_kmers = sum(kmer_dict.values())
                    '''
                    
            canonical_kmer_set = set()
            for kmer in kmer_set: 
                rc_kmer = km.reverse_complement(kmer)
                kmer_can = min(kmer, rc_kmer)
                canonical_kmer_set.add(kmer_can)
                
            uniq_kmers = len(canonical_kmer_set)
            
            
            
            if all_kmers:
                kmer_index = float(uniq_kmers)/float(all_kmers) 
            
            log_time_ram("tax %s, %d reads, %d uniq kmers, %d all kmers, %s kmer_index"%(dataset.taxid, len(taxid_reads), uniq_kmers, all_kmers, str(kmer_index)))
